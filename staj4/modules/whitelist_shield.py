...
KNOWN_BENIGN_PATTERNS = [
    ...
    # 2. OpenSSH Routine Side-Logs (Preauth Handshakes, PAM duplicate notices, Session Open/Close)
    re.compile(r"pam_unix\(sshd:auth\): (?:authentication failure|check pass; user unknown)"),
    re.compile(r"sshd\[\d+\]: (?:Connection closed by authenticating user|Connection reset by|Received disconnect from|Disconnected from authenticating user|Disconnected from invalid user).*\[preauth\]"),
    re.compile(r"sshd\[\d+\]: pam_unix\(sshd:session\): session (?:opened|closed) for user"),
    re.compile(r"sshd\[\d+\]: (?:Postponed publickey|Found matching RSA key|Found matching ED25519 key)"),

    # 3. Systemd & Kernel Events
    ...
    # 4. Package Management & Updates (Apt, Dpkg, Snap, PackageKit)
    ...
    # 5. Certificate Renewal & Scheduled Maintenance (Certbot, Logrotate, Cron Daily)
    ...
    # 6. Observability, Healthchecks & Metrics (Prometheus, KubeProbe)
    ...
    # 7. Database Maintenance Checkpoints (PostgreSQL, MySQL, Redis)
    ...
    # 8. Safe Sudo Administration Commands
